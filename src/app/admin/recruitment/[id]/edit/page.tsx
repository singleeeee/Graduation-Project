...
export default function EditRecruitmentPage() {
  ...
  // Hooks for dropdowns
  const { data: clubs = [], isLoading: isClubsLoading } = useClubsForSelection()
  const { data: registrationFields = [], isLoading: isFieldsLoading } = useRegistrationFieldsForSelection()

  const { register, handleSubmit, formState: { errors }, watch, setValue, getValues, reset } = useForm<CreateRecruitmentBatchRequest>({
  ...
  React.useEffect(() => {
    if (recruitment) {
      reset({
        ...
        startTime: recruitment.startTime ? new Date(recruitment.startTime).toISOString().slice(0, 16) : '',
        endTime: recruitment.endTime ? new Date(recruitment.endTime).toISOString().slice(0, 16) : '',
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="mt-2 text-blue-600 hover:underline">
            返回
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">编辑招新批次</h1>
