...
import type { CreateRoleRequest, UpdateRoleRequest, Role, RoleListParams, Permission, AssignPermissionsRequest, AddPermissionsRequest, RemovePermissionsRequest } from '@/lib/api'
...
export default function RolesManagementPage() {
  ...
  const { data: rolesData, isLoading: rolesLoading, error: rolesError } = useQuery({
    queryKey: ['roles', { page: currentPage, limit: pageSize, search: searchTerm }],
    queryFn: async () => {
      try {
        ...
        return response?.data || response?.data?.data || response?.roles || response?.data?.roles || []
      } catch (error) {
  ...
  const { data: permissionsData, error: permissionsError } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const response: any = await permissionsApi.getPermissions({ limit: 100 })
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* 权限管理对话框 */}
      {selectedRole && (
        <Dialog open={isPermissionDialogOpen} onOpenChange={setIsPermissionDialogOpen} key={`permission-dialog-${selectedRole.id}`}>
                  ...
                  {(modulePermissions as any[]).map((permission) => (
                    <div key={permission.id} className="flex items-center space-x-2">
                      <input
                        ...
                        checked={formData.permissionCodes?.includes(permission.code) || false}
                        onChange={(e) => {
                          const currentCodes = formData.permissionCodes || []
                          if (e.target.checked) {
                            setFormData({
                              ...formData,
                              permissionCodes: [...currentCodes, permission.code]
                            })
                          } else {
                            setFormData({
                              ...formData,
                              permissionCodes: currentCodes.filter(code => code !== permission.code)
                            })
                          }
                        }}
                        className="w-4 h-4"
...
function PermissionManagementDialog({ 
  ...
  const handleSavePermissions = () => {
    const currentPermissionCodes = Array.isArray(rolePermissionsData) ? rolePermissionsData : []
    const newPermissionCodes = Array.from(selectedPermissions)
    
    // 找出需要新增的权限代码
    const permissionsToAdd = newPermissionCodes.filter((code: string) => !currentPermissionCodes.includes(code))
    // 找出需要移除的权限代码
    const permissionsToRemove = currentPermissionCodes.filter((code: string) => !newPermissionCodes.includes(code))
    ...
    <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
      ...
      <div className="flex justify-end space-x-2 mt-4">
        ...
        <Button 
          ...
          disabled={addPermissionsMutation.isPending || removePermissionsMutation.isPending}
        >
          {(addPermissionsMutation.isPending || removePermissionsMutation.isPending) ? '保存中...' : '保存权限'}
        </Button>
      </div>
    </DialogContent>
  )
